...
export async function stepUp(db, id) {
  // Step-up also unlocks the credential vault (Task 7.3): fresh 2FA is
  // the canonical "vault open" signal. Subsequent credential interactions
  // slide vault_unlocked_at forward via lib/auth/vault-lock.js#unlockVault;
  // step_up_at itself is only renewed by another 2FA.
  await sql`
    UPDATE sessions
       SET step_up_at = now(),
