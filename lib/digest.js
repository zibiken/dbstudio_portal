...
// Per-event-type extra discriminators beyond (recipient, eventType, customerId).
// Each listed key is read from `item.metadata` and threaded into
// findCoalescable's metadataMatch so coalescing only collapses rows that
// share the same value. Without this, e.g. two phases toggled in the
// same digest window collapsed into one line carrying the first phase's
// label (spec Decision 5 deviation discovered post-Phase-G).
export const COALESCING_DISCRIMINATORS = Object.freeze({
  'phase_checklist.toggled': Object.freeze(['phaseId']),
});
...
export async function recordForDigest(tx, item, opts = {}) {
  ...
  if (COALESCING_EVENTS.has(item.eventType)) {
    const discriminators = COALESCING_DISCRIMINATORS[item.eventType];
    const metadataMatch = discriminators
      ? Object.fromEntries(discriminators.map((k) => [k, item.metadata?.[k] ?? null]))
      : null;
