...
// Escape PG LIKE metacharacters so prefixes like 'customer_user.' match the
// literal trailing underscore (not "any single character"). Mirror of the
// audit-query.js helper (M9 review M3).
...
export async function listActivityForCustomer(db, customerId, opts = {}) {
  ...
  const prefixes = Array.isArray(opts.actionPrefixes) && opts.actionPrefixes.length > 0
    ? opts.actionPrefixes.map((p) => `${escapeLikePrefix(p)}%`)
    : null;
