...
//
//   invoices
//     count       all invoices on file (excluding void)
//     latestAt    MAX(created_at) — invoices have no updated_at
//     unreadCount payment-driven: counts every invoice whose
//                 SUM(invoice_payments.amount_cents) < invoices.amount_cents
//                 AND whose raw status is neither 'paid' nor 'void'.
//                 Mirrors the row-level STATUS_COMPUTED_EXPR in
//                 domain/invoices/repo.js so the dashboard count
//                 agrees with the per-row pill (an invoice marked
//                 'paid' OR with payments meeting the total drops out
//                 of the "needs attention" count).
//
//   projects
    ...
    SELECT
      ...
      (SELECT COUNT(*)::int FROM invoices i
        ...
        WHERE i.customer_id = ${customerId}::uuid
          ...
          AND p.paid_cents < i.amount_cents)
        AS inv_unread,
