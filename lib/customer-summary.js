...
//
//   documents
//     count       only category='generic' (NDA + invoice categories are
//                 surfaced under their own tabs and would double-count
//                 here otherwise)
//     latestAt    MAX(uploaded_at) over the same generic filter
...
export async function getCustomerDashboardSummary(db, { customerId } = {}) {
  ...
  const r = await sql`
    SELECT
      ...
      (SELECT COUNT(*)::int FROM documents WHERE customer_id = ${customerId}::uuid AND category = 'generic')
        AS docs_count,
      (SELECT MAX(uploaded_at) FROM documents WHERE customer_id = ${customerId}::uuid AND category = 'generic')
        AS docs_latest,
