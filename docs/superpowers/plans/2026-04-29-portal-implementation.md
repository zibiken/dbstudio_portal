...
**Latest commit on main:** `3cc15e4` — `fix(m7): close M7 → M8 review — decrypt-failure forensic audit + 4 minors (review I1+M1+M2+M4+M5)`.

**Resume here:** **M7 complete + M7 → M8 review cleared.** Reviewer raised 0 Critical + 1 Important + 7 Minor; closed I1 + M1 + M2 + M4 + M5 in `3cc15e4`. Deferred per reviewer's classification: M3 (defence-in-depth customerId on cancelByAdmin/markNeedsUpdate — admin authority global by design, not a security boundary), M6 (route err.message verbatim — M9 polish scope), M7 (missing vault CRUD routes — intentional M8 scope). Next: begin **M8 — Invoices + NDA** (Task 8.1 `domain/invoices/{repo,service}.js` + admin/customer routes; `overdue` is computed in SQL not stored; 8.2 `templates/nda.html` + `scripts/bootstrap-templates.sh`; 8.3 NDA generator via Mustache → portal-pdf IPC). Plan: `# M8 — Invoices + NDA` starts around line 3078. Run integration tests via `sudo bash scripts/run-tests.sh`.

**Test/coverage state right now:** 445 tests green + 1 skipped (live-smoke RUN_LIVE_EMAIL gate) across 49 files, smoke.sh 5/5. Coverage on M7 modules: `domain/credentials/**` ≥80% gate cleared (97.6%+ stmts); `lib/auth/vault-lock.js` 100/100/100/100; `lib/activity-feed.js` ~100% lines; `domain/credential-requests/**` 87.3/78.0/92.0/89.8. **HTTP-layer routes for credential-requests at ~7-9%** — chosen surface was service-layer workflow tests; HTTP route tests can be added in M9 polish. Six M7 commits (range `3723156..3cc15e4`) are worth re-reading on context swap: `3723156` (Task 7.1, provider seed), `6c1d764` (Task 7.2, credentials domain incl. spec deviation for admin edit/delete), `e79bf2c` (Task 7.3, vault auto-lock), `ae75a28` (Task 7.4, credential-request workflow), `41856a7` (Task 7.5, activity-feed slice + 0007 FK SET NULL), `3cc15e4` (M7 → M8 review-fix — I1 decrypt-failure forensic audit, M1 activity-feed allow-list, M2 indexed form names, M4 view-suspended-customer docstring, M5 seed comment clarification).
