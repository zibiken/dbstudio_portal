...
| **M7** Credential vault + requests | 🚧 in progress | 2026-04-30 | 7.1 (`0005_provider_catalogue_seed.sql` — note: number bumped from plan's 0002, since 0002–0004 already shipped in M3/M5/M6; 20-row seed of curated provider suggestions with `default_fields` shaped as `[{name,label,type,required}]` where type ∈ `{text,secret,url,note}`. Catalogue is a **suggestion list for admin autocomplete**, NOT a closed set — `credentials.provider`/`credential_requests.provider` stay free-text TEXT so admins can type any name like "Combell"/"Website admin"/"M365"; 6 tests). 7.2 (`domain/credentials/{repo,service}.js` — five plan methods + three operator-instructed additions: `createByCustomer`, `createByAdminFromRequest` (FOR UPDATE on the request, encrypts with customer DEK NOT KEK directly per spec §2.4, flips status='fulfilled' + `fulfilled_credential_id`, double audit `credential.created` + `credential_request.fulfilled` both visible_to_customer), `view` (5-min step-up gate via `isStepped`, decrypts via customer DEK, audits `credential.viewed` visible_to_customer — the trust contract spelled out at spec §2.8: every admin view is timestamped, attributed, and visible to that customer), `markNeedsUpdate`, `deleteByCustomer` w/ cross-customer trap. **Spec deviation per operator instruction 2026-04-30:** the plan said "admin cannot delete in v1 — customer trust contract"; operator overrode → BOTH admin AND customer can edit (label + payload) AND delete; admin edit additionally requires step-up (defence in depth — overwriting a known-good payload is sensitive even though no plaintext is exposed); every admin action remains audit-visible-to-customer, which is the *actual* trust contract per §2.8. Adds `updateByCustomer`, `updateByAdmin` (step-up gated), `deleteByAdmin`. Edits clear `needs_update=FALSE` on payload change (the new payload IS the resolution). Audit metadata carries label + previousLabel + payloadChanged for the M9 activity feed. 32 tests across 4 files (`create.test.js` 8, `view.test.js` 7, `fulfil.test.js` 5, `manage.test.js` 12). Plaintext-leakage assertions in every audit-touching test. Coverage on `domain/credentials/**` 97.6/90.5/97.2/97.5 — well above the 80% gate. 416 tests green + 1 skipped, no regressions. |
| **M8** Invoices + NDA | — | — | |
| **M9** Profile + activity + polish | — | — | |
| **M10** Backups + go-live (gates) | — | — | |

**Latest commit on main:** `6c1d764` — `feat(m7): credential vault domain layer (Task 7.2 + spec deviation)`.

**Resume here:** Tasks 7.1 (`3723156`) and 7.2 (`6c1d764`) **complete**. Begin Task 7.3 — vault auto-lock (`lib/auth/vault-lock.js`, session-scoped 5-min idle credential-interaction window separate from step-up; flag cleared on idle, next view requires fresh step-up). Then 7.4 credential-request workflow (`domain/credential-requests/{repo,service}.js` + admin/customer routes, including the `not_applicable` reason flow), 7.5 customer activity-feed slice for vault, 7.6 coverage gate confirmation. Run integration tests via `sudo bash scripts/run-tests.sh`.

**Test/coverage state right now:** 416 tests green + 1 skipped (live-smoke RUN_LIVE_EMAIL gate) across 45 files, smoke.sh 5/5. Coverage on `domain/credentials/**` 97.6/90.5/97.2/97.5 — well above the 80% gate from `vitest.config.js`. Coverage on M6 modules unchanged from the M6 → M7 baseline. The two new M7 commits (range `3723156..6c1d764`) are worth re-reading before 7.3 if you context-swap: `3723156` (Task 7.1 — `0005_provider_catalogue_seed.sql` w/ 20 free-text-suggestion rows + `default_fields` shape), `6c1d764` (Task 7.2 — credentials domain layer including operator-instructed spec deviation: `updateByCustomer`/`updateByAdmin`/`deleteByAdmin` added on top of the plan's 5 methods).
