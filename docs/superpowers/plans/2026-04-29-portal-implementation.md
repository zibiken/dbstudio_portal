...
| **M4** Email pipeline | 🟡 in progress | 2026-04-29 | Gate M4-A closed (live MailerSend send → HTTP 202, Message-Id `69f237c1240785931dfc7c72`, delivered to bram@roxiplus.es; key `portal-v1` last-4 `GEME`). Tasks 4.1 ✅ (`lib/email.js` single-attempt client, 100/100/100/100), 4.2 ✅ (14 brand-faithful EJS templates + build pipeline + globalSetup + Madrid DD/MM/YYYY), pre-existing M3 test-isolation bug fixed (`dff8b34`), pre-existing `package.json` perm regression fixed. **Resume at Task 4.3** (outbox worker). 235 tests green, smoke 5/5. |
...
**Latest commit on main:** `342b259` — `feat: lib/dates.js — DD/MM/YYYY (Madrid) shared by emails and pages`.

**Resume here:** M4 Task 4.3 — outbox worker (`domain/email-outbox/repo.js` + `domain/email-outbox/worker.js`). The plan's contract: `tickOnce({ db, mailer, log, batchSize=10 })` claims rows with `SELECT … FOR UPDATE SKIP LOCKED`, sets `status='sending'` + increments `attempts`, renders the template, calls `mailer.send`, then either `markSent` (`status='sent'`, `sent_at=now`, `last_error=null`, drop sensitive locals via `locals = locals - 'code'`) or `markFailed` with backoff `min(2 ** attempts * 60_000, 3_600_000)` and `status` flipping back to `'queued'` if `e.retryable` else `'failed'`. After 5 attempts the row stays `'failed'`. `startWorker(deps)` registers a 5-second `setInterval` and returns the `clearInterval` shutdown. Wire into `server.js` after Fastify is built; add graceful shutdown. Tests in `tests/integration/email-outbox/worker.test.js` (3 enqueued → tick → all `status='sent'`, idempotency keys preserved, forced retryable failure backs off and re-queues, 5-attempt cap → `'failed'`).

**Test/coverage state right now:** 235 tests green across 30 files, smoke.sh 5/5, coverage on `lib/email.js` 100/100/100/100. Five session commits worth re-reading before Task 4.3 if you context-swap: `7d9b76d` (operator gate closure + the `.env` quoting trap), `dff8b34` (test-isolation fix in login-flow), `b0b220e` (mailer single-attempt contract — worker owns retry semantics), `c5b33d8` (template renderer + `_compiled.js` manifest pattern + brand tokens), `342b259` (`lib/dates.js` shared by emails and views — the worker's templates already use it via HELPERS).
