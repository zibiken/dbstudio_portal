...
**Latest commit on main:** `fabe44d` — `feat(m8.7): drop strict single-page guard + multi-page e2e (Tasks 4+5)`. Plus the upcoming Task 6 docs commit.

**Resume here:** **M8 complete; begin M8 → M9 review checkpoint.** All six M8 implementation tasks done (8.1 invoices ingest + status, 8.2 NDA template + bootstrap, 8.3 lib/nda render+sha256, 8.4 NDA draft generator + admin route + customer-edit form for representante fields, 8.5 signed/audit-trail upload, 8.6 customer NDA list). Two M8 surface-level scope deltas vs plan (operator-driven, both documented in commit messages): (a) **invoice generation** is NOT in scope — invoices are produced by a separate billing platform and only ingested here as PDF + metadata; (b) **`nda-ready` email** is NOT enqueued — drafts are admin-only and the customer signs through the operator's secure platform, getting the signed copy via this portal's NDA list only. Run review against the M8 diff `3cc15e4..91b4207` next.

**Test/coverage state right now:** 499 tests green + 2 skipped (live-email RUN_LIVE_EMAIL gate + NDA e2e RUN_PDF_E2E gate) across 56 files, smoke.sh 5/5. New/changed surfaces in M8 with their coverage posture: `domain/invoices/**` (18 service tests covering create + setStatus + listForCustomer + listAll + findById + the no-op same-status case + transition matrix; route layer 2 tests). `domain/ndas/**` (9 service tests covering happy + same-template-same-vars-identical-sha + one-char-template-edit-flips-sha + missing-rep-fields + overflow + IPC-crash + sha-mismatch + unknown-project + suspended-customer; 7 service tests for attachUploadedDocument; 3 customer-visibility tests). `lib/nda.js` (9 unit tests). `scripts/bootstrap-templates.sh` (6 integration tests). `migrations/0008_customer_representative.sql` applied. **Coverage gate** in vitest.config.js currently lists only `lib/crypto/**` 100% + `lib/auth/**` + `domain/credentials/**` at 80%. NDA + invoice modules are NOT yet under a coverage gate — should be added in the M8 → M9 review-fix bundle (recommend ≥80% line on `domain/invoices/**` + `domain/ndas/**` + `lib/nda.js` per the M2 §6 testing strategy). HTTP-layer route tests for invoice + NDA admin/customer routes are minimal (1-2 happy paths) — sufficient for the service-layer-coverage-is-the-gate convention; M9 polish can backfill if reviewer asks for HTTP-layer rigor. M7 review-deferred items still pending (review there): M3 (defence-in-depth customerId on cancelByAdmin/markNeedsUpdate), M6 (route err.message verbatim — M9 scope), M7 (vault CRUD routes — also pending; closely related to M9 customer credential view path).

**Latent IPC bug fixed in M8.4 worth flagging:** `pdf-service.js` `createServer()` defaulted `allowHalfOpen=false`, so the server's writable side auto-FIN'd on receiving the client's FIN, dropping any reply written after `await render()`. The unit test for `lib/pdf-client.js` (3 tests) used a custom mock server that handled half-open differently and never caught it. M2 → M7 never actually exchanged a real request over the socket; M8.4 was the first end-to-end use. Fix: `createServer({allowHalfOpen: true}, ...)`. The IPC contract is now correct and the e2e test gated on RUN_PDF_E2E=1 confirms it.
