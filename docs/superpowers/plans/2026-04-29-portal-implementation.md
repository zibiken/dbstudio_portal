...
**Latest commit on main:** `41856a7` — `feat(m7): customer activity-feed slice for vault (Task 7.5)`.

**Resume here:** **M7 complete.** Begin **M7 → M8 review checkpoint** — request a code review across the five M7 commits (`3723156`, `6c1d764`, `e79bf2c`, `ae75a28`, `41856a7`) plus the deferred-pointer commit `c599457`. The plan's M7 → M8 verification list (line ~3072): (a) `sudo -u portal-pdf psql portal_db` fails — confirm two-service split holds; (b) admin viewing a credential without step-up is rejected (302 to step-up via the route layer once it lands; today the service layer throws StepUpRequiredError); (c) customer sees the admin's view in their activity feed within 1 s — `lib/activity-feed.js` returns it newest-first with sub-second latency in the workflow test. After review, begin M8 — Invoices + NDA. Run integration tests via `sudo bash scripts/run-tests.sh`.

**Test/coverage state right now:** 443 tests green + 1 skipped (live-smoke RUN_LIVE_EMAIL gate) across 48 files, smoke.sh 5/5. Coverage on M7 modules: `domain/credentials/**` 97.6/90.5/97.2/97.5 (gate ≥80% ✓); `lib/auth/vault-lock.js` 100/100/100/100; `lib/activity-feed.js` 100/81.25/100/100; `domain/credential-requests/**` 87.3/78.0/92.0/89.8. **HTTP-layer routes for credential-requests at ~7-9%** — chosen surface was service-layer workflow tests; HTTP route tests can be added in M9 polish. Five M7 commits (range `3723156..41856a7`) are worth re-reading on context swap: `3723156` (Task 7.1, provider seed), `6c1d764` (Task 7.2, credentials domain incl. spec deviation for admin edit/delete), `e79bf2c` (Task 7.3, vault auto-lock), `ae75a28` (Task 7.4, credential-request workflow), `41856a7` (Task 7.5, activity-feed slice + 0007 FK SET NULL).
