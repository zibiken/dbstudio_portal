...
| **M4** Email pipeline | ✅ done | 2026-04-29 | Gate M4-A closed; 4.1 (`lib/email.js` single-attempt client) → 4.2 (14 EJS templates + build pipeline) → 4.3 (`domain/email-outbox/{repo,worker}.js` claims via SELECT FOR UPDATE SKIP LOCKED, exponential backoff capped at 1h, 5-attempt cap, drops `locals.code` on success, `startWorker` wired into `server.js` entrypoint only) → 4.4 (`tests/integration/email/live-smoke.test.js` gated on RUN_LIVE_EMAIL=1, RUNBOOK procedure documented) → 4.5 (`service.create` enqueues `admin-welcome`, `requestPasswordReset` enqueues `admin-pw-reset`; 15th template `admin-welcome.ejs` added — small, derived from customer-invitation but worded for admins; create-admin.js stdout demoted to fallback). Brand: dbstudio.one logo + favicon set wired into all three layouts (`public/brand/`, three /favicon /apple-touch-icon /site.webmanifest routes). 248 tests green + 1 skipped (live-smoke). Coverage on domain/email-outbox/** 100/75/100/100; lib/auth/** + domain/admins/** still ≥ 80 every metric. **Pause for M4 → M5 review checkpoint.** |
...
**Latest commit on main:** `7180fe1` — `feat(m4): admin auth flows enqueue email instead of stdout-only (Task 4.5)`.

**Resume here:** M4 → M5 review checkpoint, then begin M5 — Customer create + onboarding (Tasks 5.1–5.5: `domain/customers/repo.js`+`service.js` with the per-customer DEK envelope, admin customer-list view, customer onboarding routes mirroring admin auth, customer dashboard stub, suspend/archive). Operator side-task: run `tests/integration/email/live-smoke.test.js` with `RUN_LIVE_EMAIL=1` on the box and confirm inbox arrival + DKIM pass for `dbstudio.one` (deferred from 4.4 — the test is committed but the live send is operator-driven).

**Test/coverage state right now:** 248 tests green + 1 skipped (live-smoke RUN_LIVE_EMAIL gate) across 32 files, smoke.sh 5/5. Coverage on `lib/email.js` + `domain/email-outbox/repo.js` 100/100/100/100, `domain/email-outbox/worker.js` 100/60/100/100 (branch gap is defensive nullish coalescing on Error fields and DB-default columns, not worth synthesising). `domain/admins/**` 95.79/80.89/100/98.19; `lib/auth/**` 98.49/96.07/95.34/98.26 — all ≥ 80 every metric. Five session commits worth re-reading before M5 if you context-swap: `1542cb8` (Task 4.3 outbox worker — claim/render/send/retry single-tx contract), `4261964` (brand: dbstudio.one logo + favicon wired into all layouts), `38ac84f` (Task 4.4 live-send smoke + RUNBOOK email-outbox section), `7180fe1` (Task 4.5 admin-welcome + admin-pw-reset enqueue, 15th template), and `c5b33d8` (template renderer manifest pattern — read once before adding any new template in M5+).
