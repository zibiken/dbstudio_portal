...
**Latest commit on main:** `a855416` — `fix(m6): close M6 → M7 review — multipart truncation + audit fail-open + rate limit + project edit (review C1+C2+I1+I2+I7+M4)`.

**Resume here:** M6 → M7 review **cleared** (2C+4 of 7I+1 of 7M fixed in `a855416`; remaining minors and I3/I5/I6 deferred per reviewer's classification — none load-bearing for M7 unblock). Begin M7 — Credential vault + requests (Tasks 7.1–7.x: `migrations/0002_provider_catalogue_seed.sql`, `domain/credentials/{repo,service}.js` w/ DEK-encrypted payloads, admin credential issuance + view-with-step-up flow, customer credential creation/fulfilment + 5-minute idle auto-lock, audit-visible-to-customer on every admin view of a credential — the trust contract). Run integration tests via `sudo bash scripts/run-tests.sh`.

**Test/coverage state right now:** 378 tests green + 1 skipped (live-smoke RUN_LIVE_EMAIL gate) across 41 files, smoke.sh 5/5. Coverage on `lib/files.js` 100/100/100/100, `domain/documents/service.js` 92/82/100/93, `routes/customer/documents.js` 89/91/100/89, `routes/public/files.js` 86/72/100/84, `routes/admin/documents.js` 71/54/100/78 (catch-block branches will be exercised by 7.x credential-fulfilment HTTP tests that share the same shape). The six M6 commits (range `dc147dc..a855416`) are worth re-reading before M7 if you context-swap: `dc147dc` (Task 6.1 lib/files.js), `4e0056b` (Task 6.2 upload pipeline + multipart wiring), `e311d43` (Task 6.3 download flow + migration 0004 + /files/* wildcard route gotcha), `2aa5046` (Task 6.4 projects CRUD), `e81f8fd` (Task 6.5 version chain w/ cross-customer parent guard), `a855416` (M6→M7 review — C1 multipart truncation, C2 integrity-audit fail-open, I1 60/min rate limit, I2 project edit route, I7 token length cap, M4 tx-wrap audit-trigger toggle in tests).
