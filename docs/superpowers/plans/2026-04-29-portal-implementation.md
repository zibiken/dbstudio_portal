...
| **M10** Backups + go-live (gates) | — | — | Implementer-side: `scripts/backup.sh` (pg_dump → tar storage → age encrypt → rclone push to Hetzner Storage Box → 30/12/5 retention), `scripts/restore-drill.sh` (random backup → decrypt with offline age key → pg_restore into scratch DB → unwrap one customer DEK → decrypt one credential), `systemd/portal-backup.{service,timer}` (02:30 UTC nightly oneshot under portal-app), expanded `scripts/smoke.sh` (MailerSend reachable + signed-URL round-trip + portal-pdf hello probe + migration ledger up-to-date). Operator-side gates: 10-A (Hetzner Storage Box provision + rclone config + age keypair on workstation, public key onto server, second physical-medium copy of the private key) → 10-B (run `backup.sh` once + enable timer + run `restore-drill.sh`, document outcome) → 10-C (Cloudflare DNS A/AAAA proxied, NPM origin swap from coming-soon to 127.0.0.1:3400, walk the production smoke as the bootstrap admin). Plan section starts around line 3307. |

**Latest commit on main:** `9a02ad1` — `docs(m9-review): refresh resume pointer post review-fix bundle`.

**Resume here:** **M9 + M9 → M10 review-fix bundle complete and pushed.** Code-reviewer pass found 0 Critical, 2 Important (I1 + I2), 1 deferred-Important (I3 verify-bounce UX), 9 Minors. All Important items + 3 Minors landed in `3de55d8`; the rest fold into `docs/superpowers/follow-ups.md` as v1.1 work per the reviewer's calibration. **Conditionally green for M10** per the reviewer; 565 tests green + 2 skipped across 67 files; smoke 5/5. Next: **M10 — Backups + go-live**. Implementer-side work (`backup.sh`, `restore-drill.sh`, systemd timer, expanded smoke) can land any time; the three operator gates (10-A storage box + age keypair, 10-B restore drill, 10-C Cloudflare DNS + NPM origin swap) need the user at the keyboard.
