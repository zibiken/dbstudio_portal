...
| **M5** Customer create + onboarding | ✅ done | 2026-04-29 | 5.1 (`domain/customers/{repo,service}.js` — `service.create` is fully transactional: customers row + per-customer DEK wrapped under KEK + first `customer_users` row with hashed invite token + audit + `customer-invitation` outbox enqueue, all in one `db.transaction()`. Atomicity proven via `customer_users.email` UNIQUE collision.) → 5.2 (`routes/admin/customers.js` + 5 EJS views: list w/ pagination + ILIKE search on `razon_social`/`nif`, new w/ CSRF, detail, created confirmation page that surfaces the plaintext invite URL once for the belt-and-braces stdout fallback, not-found.) → 5.3 (`routes/customer/onboarding.js` + 4 views — customer welcome on `/customer/welcome/:token` (split from admin `/welcome/:token`); `domain/customers/service.completeCustomerWelcome` unwraps the customer DEK with KEK at request time, encrypts the TOTP enrol secret with the **DEK not the KEK**, persists `totp_secret_enc/iv/tag` on `customer_users`. Round-trip + 'KEK directly throws' assertion in test.) → 5.4 (`/customer/dashboard` stub w/ M6/M7/M8/M9 placeholder cards; `requireCustomerSession` hoisted to `lib/auth/middleware.js`.) → 5.5 (suspend / reactivate / archive transitions in `domain/customers/service.js` — each runs in one tx: lock customer FOR UPDATE, validate state, flip status, `revokeCustomerSessions` (suspend/archive only — reactivate leaves them revoked on purpose so customer must re-login), audit. Admin route POST `/admin/customers/:id/{suspend,reactivate,archive}` w/ CSRF. Detail-page action buttons by status.) Pre-M5 cleanup: timezone display flipped from Europe/Madrid to **Atlantic/Canary** (operator is in the Canary Islands; 1-hour offset year-round). **M5 → M6 review cleared 0 Critical + 3 Important + 1 of 5 Minor:** I1 suspend-vs-onboarding race (customer could end completeCustomerWelcome and have a never-revoked session if suspendCustomer interleaved) → fixed both ways: (a) `requireCustomerSession` joins `customers.status='active'`, (b) `createSession` + `stepUp` + `customer.login_success` audit folded into `completeCustomerWelcome`'s tx so the same commit consumes the invite AND mints the live session. I2 route-coverage gaps for reactivate/archive/422-rerender/404-bad-UUID/detail-button-matrix — 6 new tests. I3 onboarding error paths (short pw / wrong TOTP / HIBP-pwned) — 3 new tests. M2 `customer.suspended/reactivated/archived` audits now flagged `visible_to_customer=true` for the future M9 activity feed. M5 (Pino redact paths) `kek` + `inviteToken/inviteUrl/welcomeUrl/resetUrl/verifyUrl/revertUrl` (camel + snake) added for defence in depth. **Deferred minors:** M1 (URL-path tokens land in journald access log via `req.url` — admin precedent from M3, needs custom `serializers.req`), M3 (created-page `expiresAt` recomputed from clock instead of read from row — cosmetic), M4 (catch-block uses message-regex instead of `err.code === '23505'` for unique-violation detection). 280 tests green + 1 skipped, smoke 5/5. Coverage: `domain/customers/service.js` 100/82.14/100/100 (branch gap = audit nullish-fallbacks). |
| **M6** Documents + projects | ✅ done | 2026-04-30 | 6.1 (`lib/files.js` — storagePath/safeFilename/assertSize/assertCustomerQuota/mimeFromMagic/signDownloadToken w/ 100/100/100/100 coverage and 42 unit tests covering path traversal, NUL/parent-segment rejection, NFC unicode, control-char strip, magic-byte sniff, signed-token round-trip + MAC tamper + payload forge + wrong-secret + wrong-kind + 60s TTL contract). 6.2 (upload pipeline — streams into a hashing+counting+sniffing transform, atomic rename, tx with FOR UPDATE quota recheck, audit visible_to_customer; CSRF on the multipart route via x-csrf-token header set by an inline JS shim; `@fastify/multipart` transport-layer cap). 6.3 (download flow — migration `0004_download_token_consumptions.sql` for single-use enforcement at the PK layer; 5 typed errors mapping to 400/410/410/410/500; `/files/*` wildcard route NOT `:token` because find-my-way's static-suffix matching swallows the dot in `<b64u>.<b64u>` tokens — comment in the file flags this; sha256 verified BEFORE response bytes via buffer-then-send so the spec §2.7 "abort with 500 + audit" is achievable). 6.4 (projects CRUD — admin list/new/create/detail/status/edit + customer read-only list; status transitions free in v1 but every change writes a `project.status_changed` audit visible_to_customer w/ previousStatus + newStatus). 6.5 (version chain — parent_id loaded BEFORE disk I/O, cross-customer parent rejected, parent's category inherited and the caller's category arg silently overridden). **M6 → M7 review cleared 2 Critical + 4 of 7 Important + 1 of 7 Minor:** C1 multipart truncation silently producing a corrupt 50 MiB document (busboy truncates at limits.fileSize, hash matches truncated bytes, M10's verification job never flags it) → streamToTemp returns `truncated` and uploadForCustomer rejects pre-tx. C2 integrity-audit write failure swallowing the 500 → audit wrapped in try/catch + log, IntegrityFailureError throws unconditionally. I1 signed-URL issuance unbounded (spec §2.6 mandates 60/min) → checkLockout/recordFail keyed on signed_url:{admin\|customer}:<id>. I2 projectsService.update was orphaned → admin edit route + form on detail.ejs. I7 /files/* accepted unbounded path length → 4096-char cap before HMAC. M4 audit_log_block_modify trigger toggled outside a tx (test crash mid-cleanup left it disabled) → `tests/helpers/audit.js` wraps DISABLE/DELETE/ENABLE in one tx. **Deferred:** I3 CSP audit (M10 hardening), I4 parts iteration error handling (subsumed by C1), I5 SERIALIZABLE on consumeDownloadToken (current behaviour correct; M7 will revisit), I6 customerStorageBytes double-counts versions (intentional; needs docstring update), M1 spec drift MB vs MiB, M2 unbounded chain walk, M3 redundant audit metadata, M5 helper-use inconsistency, M6 traversal-regex policy doc, M7 parent_id FK policy. 378 tests green + 1 skipped, 41 files, smoke 5/5. Coverage: `lib/files.js` 100/100/100/100, `domain/documents/service.js` 92/82/100/93, `routes/customer/documents.js` 89/91/100/89, `routes/public/files.js` 86/72/100/84, `routes/admin/documents.js` 71/54/100/78. |
| **M7** Credential vault + requests | ✅ done | 2026-04-30 | 7.1 (`0005_provider_catalogue_seed.sql`; 20-row free-text-**suggestion** seed). 7.2 (`domain/credentials/{repo,service}.js` — five plan methods + three operator-instructed additions: createByCustomer, createByAdminFromRequest (FOR UPDATE on the request, encrypts with customer DEK NOT KEK directly per spec §2.4, flips status='fulfilled' + fulfilled_credential_id, double audit), view (vault-lock gate after Task 7.3, decrypts via customer DEK, audits visible_to_customer — the trust contract per spec §2.8), markNeedsUpdate, deleteByCustomer w/ cross-customer trap. **Spec deviation per operator 2026-04-30:** plan said "admin cannot delete in v1"; operator overrode → BOTH admin and customer can edit (label + payload) AND delete; admin edit step-up gated; audit-visible-to-customer remains the actual trust contract. Adds updateByCustomer, updateByAdmin (step-up), deleteByAdmin). 7.3 (`lib/auth/vault-lock.js` + `0006_session_vault_unlocked_at.sql` — session-scoped sliding-idle 5-min flag separate from step-up; stepUp also sets vault_unlocked_at; view's gate switched from isStepped → isVaultUnlocked, refresh on success, no-refresh on locked-vault rejection (a leaked sid cannot keep itself alive by hammering the route). 9 tests, 100/100/100/100 coverage). 7.4 (`domain/credential-requests/{repo,service}.js` + admin/customer routes + 7 EJS views — createByAdmin with field-shape validation incl. duplicate-name dedupe + allowed-types {text,secret,url,note}, fulfilByCustomer encrypts via customer DEK + double audit, markNotApplicableByCustomer requires non-empty reason, cancelByAdmin open-only. Customer detail view dynamically renders per-field input by type. 14 tests, 87.3% domain coverage). 7.5 (`lib/activity-feed.js` listCredentialActivityForCustomer + `0007_cr_fulfilled_credential_id_set_null.sql` — customer-safe slice of audit_log scoped via metadata.customerId, joined to admins.name and customer_users.name for actor_display_name (admins.email NEVER reaches customer slice), strips ip + user_agent_hash + audit-internal 'tag' field, newest-first, default 50, hard cap 200. 0007 lets credential deletion preserve request history while clearing the dangling FK. 4 tests, 100% line coverage). 7.6 — coverage gate cleared: domain/credentials/** 97.6/90.5/97.2/97.5; lib/auth/vault-lock.js 100/100/100/100; lib/activity-feed.js 100/81.25/100/100; domain/credential-requests/** 87.3/78.0/92.0/89.8. **HTTP-layer route coverage NOT exercised** (admin + customer credential-request routes ~7-9% — service-layer workflow test is the chosen surface; HTTP routes will be exercised in M9 polish). **M7 → M8 review cleared 0 Critical + 1 Important + 4 of 7 Minor in `3cc15e4`:** I1 — `view` lacked a forensic audit on decrypt failure → mirrors M6 documents file_integrity_failure: `credential.decrypt_failure` audit visible_to_customer=FALSE outside the rolled-back tx, `DecryptFailureError` thrown, audit-write try/catch'd so a downstream DB blip doesn't swallow the throw. M1 activity-feed metadata deny-list → 10-key `SAFE_METADATA_KEYS` allow-list. M2 admin-form parallel arrays silently desync on unchecked checkboxes → indexed names + `field_count` hidden. M4 view doesn't gate customer.status → docstring documents this as intentional admin-forensic carve-out (customer-side view path in M8 MUST add the check). M5 0005 seed comment clarified. **Deferred per reviewer:** M3 defence-in-depth customerId on cancel/markNeedsUpdate (admin authority is global; route layer already gates), M6 route renders err.message verbatim (M9 polish scope), M7 missing vault CRUD routes (intentional M8 scope). Six M7 commits (`3723156` → `3cc15e4`) + plan-update `7ad513c`; 445 tests green + 1 skipped across 49 files; smoke 5/5; portal.service restarts cleanly. |
| **M8** Invoices + NDA | ✅ done | 2026-04-30 | 8.1 (`domain/invoices/{repo,service}.js` — invoice ingest + status transitions + SQL-computed overdue; admin uploads PDF + metadata, customer never generates an invoice; route at `/admin/customers/:id/invoices/new` streams via M6 upload pipeline + creates invoice row; status transitions: open↔paid, open↔void, paid↔void refused → route through 'open'; 18 service tests + 2 route tests). 8.2 (`templates/nda.html` — verbatim copy of operator-supplied legal counsel text "Acuerdo de Confidencialidad Mutuo y Protección de Datos", 412 lines, 9 Mustache placeholders identified; `scripts/bootstrap-templates.sh` strips Google Fonts preconnects + @import, inlines local @font-face referencing `/var/lib/portal/fonts/cormorant-garamond-500.woff2`; 6 tests). 8.3 (`lib/nda.js` — Mustache + sha256 with strict typo-guard + missing-key + non-string rejection; `NDA_PLACEHOLDERS` exported as canonical list; 9 unit tests covering identical-input → identical-sha auditability invariant + escape behavior + Mustache UTF-8 quirks). 8.4 (`migrations/0008_customer_representative.sql` adds 3 nullable `representante_*` cols on `customers`; `domain/customers/service.updateCustomer` + admin edit form at `/admin/customers/:id/edit`; `domain/ndas/{repo,service}.js` `generateDraft` does load → validate → render → IPC → quota-recheck-FOR-UPDATE → write disk → tx insert documents+ndas+audit; **drafts are admin-only** per operator scope clarification: `visible_to_customer=FALSE` on every draft audit, customer download path explicitly refuses `category='nda-draft'`; `LUGAR_FIRMA='Adeje'` fixed, `FECHA_FIRMA=euDate(today)` auto; route at `/admin/customers/:id/ndas/new` with project picker + missing-rep-field warning; **latent IPC half-open bug fixed in `pdf-service.js`**: `createServer({allowHalfOpen: true})` so the server's writable side stays open through async render; 9 service tests + 1 e2e test gated on RUN_PDF_E2E=1; RUNBOOK M0-B-pdf section added with Chromium apt-deps + Cormorant Garamond scp recipe). 8.5 (`attachUploadedDocument({ndaId, documentId, kind})` in `domain/ndas/service.js` — admin uploads signed + optional audit-trail PDF via two POSTs to `/admin/ndas/:id/upload-{signed,audit}`; reuses M6 upload pipeline + follow-up tx attach; FOR UPDATE on ndas, slot-empty check, customer-id + category match guards; audit `nda.signed_uploaded` / `nda.audit_trail_uploaded` `visible_to_customer=TRUE` so customer activity feed surfaces both milestones; 7 tests). 8.6 (`routes/customer/ndas.js` GET `/customer/ndas` + `views/customer/ndas/list.ejs`; `listNdasForCustomer` filters `signed_document_id IS NOT NULL` so drafts NEVER reach the customer surface; projection excludes `generated_by_admin_id` + `draft_document_id`; downloads via existing `/customer/documents/:id/download` with M6 rate-limit + signed-URL + draft-category refusal; 3 tests). **Operator scope deltas:** plan said "Enqueue nda-ready email" — DROPPED, customer signs externally and gets the signed copy via this portal only; plan said "admin uploads via M6 documents form" for invoices — ADDED dedicated invoice form (still uses M6 plumbing, captures metadata at upload). 499 tests green + 2 skipped (live-email + PDF e2e), 56 files, smoke.sh 5/5. **Operator gates flagged in RUNBOOK before production NDA generation:** (1) `apt install libnspr4 libnss3 libxcomposite1 libxdamage1 libxfixes3 libxrandr2 libgbm1 libxkbcommon0 libpango-1.0-0 libcairo2 libasound2t64 libatk1.0-0 libatk-bridge2.0-0 libatspi2.0-0 libdrm2`; (2) `scp` Cormorant Garamond 500 woff2 to `/var/lib/portal/fonts/cormorant-garamond-500.woff2`; (3) `sudo bash scripts/bootstrap-templates.sh`. Six M8 commits (range `e5e095a..91b4207`): `e5e095a` (8.1 invoices), `98ae975` (8.2 template + bootstrap), `73470b6` (8.3 lib/nda), `7f95e27` (8.4 generator + IPC fix + customer-edit form + RUNBOOK), `e6c7cb4` (8.5 signed upload), `91b4207` (8.6 customer visibility). |
| **M8.7** Multi-page NDA print design | ✅ done | 2026-04-30 | Multi-page NDA rendering: verbatim legal template now renders across however many A4 pages content needs. Spec at `docs/superpowers/specs/2026-04-30-m8-7-multi-page-nda-design.md`. CSS: `@page { margin: 20mm 18mm 20mm 25mm }` (Spanish-legal proportions, binding-edge left), body 9.5pt + line-height 1.4, headings 10.5pt, `page-break-inside: avoid` + `break-inside: avoid` on every numbered clause + parties block + signature block, `page-break-after: avoid` on clause headings (no orphan headings). Signature block redesigned: closing paragraph + two 78×40mm rectangles in a flexbox row, each carrying `data-yousign-anchor="provider"` / `"client"` for Yousign's signature-widget placement (UI click target now, future API anchor). pdf-service.js: `displayHeaderFooter: true` with continuation `headerTemplate` ("Acuerdo de Confidencialidad — <razón social>", interpolated from `options.continuationTitle`) + footer `Página <pageNumber> de <totalPages>`. Removed `scrollHeight > A4_HEIGHT_PX` overflow check + the offending-field selector loop. **Latent Puppeteer-24 bug fixed**: `page.pdf()` returns `Uint8Array`, not `Buffer`; `Uint8Array.toString('base64')` returns a comma-joined byte list, NOT base64, so the client-side sha-mismatch guard fired on every multi-page render. Wrap as `Buffer.from(pdf.buffer, ...)` before hashing + encoding. `lib/pdf-client.js` default timeoutMs 30s → 60s for multi-page + Chromium cold-start. `domain/ndas/service.js`: `NdaOverflowError` class + `nda.draft_overflow` audit path removed (unreachable now); generateDraft passes `continuationTitle: customer.razon_social` in IPC options. Tests: 18 mocked NDA tests (was 19, removed overflow-mock test) + 8 bootstrap tests (was 7, added `data-yousign-anchor` assertion); RUN_PDF_E2E now requires a valid multi-page PDF — verified 9/9 with realistic Spanish company data producing a ~900KB 2-page PDF in 2-3s. 499 mocked tests green + 2 skipped (live email + RUN_PDF_E2E gate); coverage gates pass; smoke 5/5. Six M8.7 commits: `a5eca14` (Task 1, CSS), `37e87db` (Task 2, signature rectangles), `36a5d5f` (Task 3, headerTemplate + footerTemplate), `fabe44d` (Tasks 4+5, drop guard + Buffer.from sha-fix + multi-page e2e + bootstrap anchor test), `<head>` (Task 6, docs). Yousign workflow: operator clicks each rectangle in Yousign UI to anchor signature widgets — provider left, client right. RUNBOOK "M8.7 (landed)" section documents the re-bootstrap procedure for future legal-counsel template updates. |
| **M9** Profile + activity + polish | ✅ done | 2026-04-30 | 9.1 (6 customer profile sub-tasks: name, email-with-verify, password+HIBP, 2FA-TOTP regen, backup-codes regen, sessions list / revoke / log-out-everywhere — 35 service tests, all visible_to_customer audits). 9.2 (admin profile mirror — 11 service tests). 9.3 (broader customer activity feed in `lib/activity-feed.listActivityForCustomer` with metadata allow-list extended for M9 keys; routes/customer/activity + view; 7 tests). 9.4 (admin audit-log view + streamed CSV export in `lib/audit-query.{listAuditPage,streamAuditCsv}` — id-only cursor avoids the JS-Date µs-truncation pitfall, csv-escaping tested; 6 tests). 9.5 (`scripts/i18n-audit.js` — 620 offenders across 79 files; v1 ships EN-only, full localisation pass tracked in `docs/superpowers/follow-ups.md`). 9.6 (`scripts/a11y-check.js` static scanner reports 0 offenders for img-alt + heading-order + form-input-label; skip-link + focus-visible ring shipped; full axe-core run deferred per follow-ups). 9.7 (`docs/superpowers/m9-acceptance-dryrun.md` walks §11 line-by-line: 4/4 two-service split, 1/2 repo discipline (signed commits is operator policy), 1/1 NDA auditability, 5/6 M9 additions; the only acknowledged-not-green item is i18n which is deferred to v1.1). 9.X (M7 reviewer-deferred vault CRUD: customer-side credential list + create + delete via `routes/customer/credentials.js` — view-with-decrypt + edit deferred per follow-ups since they need the customer vault-unlock flow). Coming-soon polish: swap to logo-black so the brand shows on the white bg + skip-link infra. **Latent bug fixed in `lib/audit-query.js` design**: PG stores ts at µs precision, JS Date is ms; a strict-equality cursor on `(ts, id)` would silently skip rows when the JS-Date round-trip truncated µs to ms — solved by using id-only cursor (UUIDv7 ms-precision sorts identically). Twelve commits (`ff6327a..4ab2d98`) excluding the polish + push. 565 tests green + 2 skipped (RUN_LIVE_EMAIL + RUN_PDF_E2E gates) across 67 files. Smoke 5/5. Migration 0009_email_change_requests.sql applied. **M9 → M10 review-fix bundle landed in `3de55d8`** clearing 0 Critical + 2 Important + 3 of 9 Minors: I1 (security headers on hijacked CSV — `lib/secure-headers.applySecureHeadersRaw` helper, called from audit-export route on `reply.raw` before first chunk), I2 (rate-limits on every profile-mutation route both sides — pw 5/15min, email-req 3/hr, email-verify 5/5min, email-revert 5/hr-by-IP, totp-regen 5/5min, backup-regen 5/15min), M1 (`domain/customer-users/repo.js` deleted — was 0% covered dead code), M2 (credentials/new no longer echoes secret values back; type=password + blank-on-rerender), M4 (audit-query cursor regex tightened to canonical 8-4-4-4-12 UUID). Deferred to v1.1 in `docs/superpowers/follow-ups.md`: I3 (verify-bounce UX), M3 (LIKE underscore wildcards), M5 (audit_log metadata customerId index), M6 (raw err.message in delete error path), M7 (QR SVG render), M9 (i18n-audit regex backreference), M10 (TOCTOU note). |
| **M10** Backups + go-live (gates) | 🟡 paused at 10-C | 2026-04-30 | Implementer-side complete (commits `43e76e0` `6646f5a` `f52e387` `90ac74c`): `scripts/backup.sh` + RUN_BACKUP_TESTS-gated test, `systemd/portal-backup.{service,timer}` (02:30 UTC daily, Persistent=true, RandomizedDelaySec=300), `scripts/restore-drill.sh` + RUNBOOK procedure, smoke.sh expanded 5→9 (MailerSend reach, signed-URL round-trip, portal-pdf hello, migration ledger). Operator gates 10-A + 10-B closed: Hetzner Storage Box `u586696` provisioned, `portal-app` ed25519 key registered, rclone `hetzner-portal:` remote configured under portal-app's `rclone.conf` (root's `gdrive:` untouched), age 1.1.1 installed on server, age keypair generated on Windows workstation with two-medium offline custody, public key `age19xz4l9tz3mnt0a8rughm4k8t8qt4xptwtt5n3fte84eczkqkv37s7trk67` in `/var/lib/portal/.age-recipients` (mode 0440 portal-app). First nightly backup ran successfully (`20260430T170447Z`, 469KiB db dump + 10KiB storage tar landed on Storage Box). Restore drill round-trip green via `7831e0e` (workdir-perms bug caught + fixed; "scratch DB restored cleanly, no customers" branch fired correctly because no production customers existed yet). Timer enabled, next firing 2026-05-01 02:34 UTC. M10-C partially landed: Cloudflare DNS already proxied, NPM origin already at `127.0.0.1:3400`, `GET /` redirected from coming-soon to `/login` (`146f742`), 6 test-fixture admins purged, bootstrap admin `bram@roxiplus.es` / `Bram Deprez` created (id `019ddf64-c9c0-7171-aa05-cb509c579092`, MailerSend providerId `69f38e8d76de5d98a7dbe2ab` for the welcome email). **PAUSED before operator onboarding** — see M11 row. The "1 customer + 1 credential decrypted cleanly" §11 acceptance line still pending; re-runs after M11 finishes the polish work and produces a real test customer. |
| **M11** Visual redesign + TOTP QR | 🟡 14 of 21 tasks landed; T15-T21 remain | 2026-04-30 | Brainstorm → spec → 21-task implementation plan complete (`docs/superpowers/specs/2026-04-30-m11-visual-redesign-design.md`, `docs/superpowers/plans/2026-04-30-m11-visual-redesign-implementation.md`). 18 commits landed: T1 vendor tokens.css + woff2 fonts (`47cdc35`), T2 rewrite tailwind.config + app.src.css (`c590c69`), T3 layouts → new chrome shells with stub partials (`8fc90e6`), T4 leaf partials eyebrow/alert/button/input (`0984cf5`), T5 container partials card/page-header/table/breadcrumb/footer (`b67ff28`), T6 hero + chrome partials + portalVersion + inline scripts (`fcb0d67`), T7 lib/qr.js + 7 TDD unit tests (`311fdfe`), T8 _qr partial (`d3d938d`), T9 /login + /login/2fa restyled (`15877fe`), T10 admin /welcome/:token with QR (`dff8f17`), T11 /customer/welcome/:token with QR (`0262e2a`), T12 /reset + 2fa-enrol backup-codes restyled (`4a76653`), T13 admin customers list/new/detail/edit/created/not-found + sub-tab strip (`4356979`), T14a-e admin per-customer subroutes — NDAs (`bb99a6b`), documents (`4a76f73`), invoices (`ba8c041`), projects (`775880c`), credential-requests (`26644b4`). Plus 3 visual-fix passes: align components to dbstudio.one rendered values (`8dd5fd7`), split logo into thin pre-hero brand strip (`d3b296f`), and replace password-toggle middot with proper eye/eye-off SVG icon (current). 572 tests green + 3 skipped throughout, smoke 9/9 every commit. **PAUSED: T15 (admin profile + audit + export) in progress, T16-T21 pending** (`lib/customer-summary.js` + tests, customer dashboard bento, customer surface restyle, a11y-check.js extension, smoke probe #10, m11-acceptance-dryrun.md). **Operator-reported visual issues at handoff:** (a) password show/hide eye icon was rendering as `·` middle dot — fixed in current commit with feather-style inline SVG eye + eye-off icons + script that toggles between them. (b) Operator could not get past `/login/2fa` (TOTP challenge) — kept bouncing back to `/login`. Likely cause: post-2FA the redirect target is `/` which redirects to `/login` (admins have no dashboard). Needs a one-line route fix in `routes/public/login-2fa.js` to redirect to `/admin/customers` after a successful admin 2FA, OR add a `GET /admin` redirect-to-customers handler. Diagnose this FIRST in the next session before continuing T15. |

**Latest commit on main:** `26644b4` — `feat(m11-14e): admin credential-requests subroute restyled` (plus the eye-icon fix landing in this handoff commit).

**Resume here:** **M11 paused at T14 close with 14 of 21 tasks landed.** Operator onboarded via the welcome flow (consumed welcome token, set password, scanned QR, saved backup codes). Two operator-reported issues at the pause:

1. **Eye-icon fix** — fixed in this commit. The password show/hide button was rendering as `·` (HTML `&middot;`); replaced with feather-style inline SVG eye + eye-off icons that toggle on click.
2. **`/login/2fa` bounces back to `/login`** — UNRESOLVED. After entering a valid TOTP code, the operator lands on `/login` instead of an authenticated admin surface. Need to inspect `routes/public/login-2fa.js` line ~117 (the post-success redirect) — currently `reply.redirect('/', 302)` and the `GET /` handler in `server.js:119` redirects unconditionally to `/login`. Fix likely: after successful admin 2FA, redirect to `/admin/customers` (the admin home) instead of `/`. Customers route through `requireCustomerSession` to `/customer/dashboard` already, so this only affects admin sessions. Verify with the integration test `tests/integration/auth/login-flow.test.js` "full flow" — if it expects redirect to `/`, update both route + test together.

Next session priority order: (1) **fix the 2FA redirect** so the operator can actually reach the admin surface and see the new chrome land in T13/T14; (2) resume T15 (admin profile + audit + export — `routes/admin/profile.js` and `routes/admin/audit.js`); (3) T16 `lib/customer-summary.js` + integration tests (TDD); (4) T17 customer dashboard bento; (5) T18a customer NDAs/Documents/Invoices/Projects; (6) T18b customer Credentials/Credential-requests/Activity/Profile; (7) T19 extend `scripts/a11y-check.js` with 6 M11 pattern checks; (8) T20 add gated probe #10 to `scripts/smoke.sh`; (9) T21 author `docs/superpowers/m11-acceptance-dryrun.md`. Plan is at `docs/superpowers/plans/2026-04-30-m11-visual-redesign-implementation.md`.

**Test/coverage state right now:** 572 tests green + 3 skipped (RUN_LIVE_EMAIL + RUN_PDF_E2E + RUN_BACKUP_TESTS gates) across 68 files. Net +7 from end of M9: the new tests are the 7 lib/qr.js unit tests added in T7. Smoke 9/9. Coverage gates unchanged from M9. Migrations 0001-0009 applied. M11 is presentation-layer-only — no domain/migrations/lib changes that affect coverage.

**Operational follow-ups baked into RUNBOOK:**
- `M0-B-pdf-prereqs` — Chromium under sandbox + Puppeteer pipe-true workaround. Already verified working on the box.
- `M0-B-pdf` — bootstrap-templates.sh + Cormorant Garamond 500 latin + latin-ext woff2 (both fonts already on `/var/lib/portal/fonts/`).
- `M8.7 (landed)` — re-bootstrap recipe for future legal-counsel template updates (archive prior `nda.html` under `nda-<sha>.html` so prior NDAs' `template_version_sha` remain reproducible).

**Latent bugs fixed during M8 + M8.7 worth flagging on context-swap:**
1. `pdf-service.js` `createServer()` defaulted `allowHalfOpen=false` (latent since M1) — server's writable side auto-FIN'd before async `render()` finished, dropping every IPC reply. Fix: `createServer({allowHalfOpen: true}, ...)` in M8.4.
2. `pdf-service.js` had no `error` handler on the conn; client timeout's `conn.destroy()` made the server's reply-write hit EPIPE and crash portal-pdf.service. Fix in M8 review I1: `conn.on('error', () => {})` + try/catch around `conn.end(reply)`.
3. Puppeteer 24+ returns `Uint8Array` from `page.pdf()`, NOT `Buffer`. `Uint8Array.toString('base64')` returns a comma-joined byte list (NOT base64) so the client-side sha-mismatch guard fired on every multi-page render. Fix in M8.7: `Buffer.from(pdf.buffer, pdf.byteOffset, pdf.byteLength)` before encode/hash.
4. Chromium under the systemd hardening profile needed `pipe: true` in puppeteer.launch (avoid the WS-endpoint-discovery hang) + `ReadWritePaths=/var/lib/portal-pdf` in the unit (so crashpad can write its database under HOME). Fixed during M8.4 + the review-fix bundle.
