...
**Latest commit on main:** `b3e7c12` — `docs(m8.7): mark landed; RUNBOOK + plan progress (Task 6)`.

**Resume here:** **M8 + M8 → M9 review-fix bundle + M8.7 multi-page NDA all complete and pushed.** The portal can now generate real multi-page NDA PDFs end-to-end against realistic Spanish company data (verified live via RUN_PDF_E2E=1 producing a ~900KB 2-page PDF in 2-3s through the full Mustache → IPC → Puppeteer pipeline). Next milestone: **M9 — Profile + activity + polish** (plan section starts around line 3187). M9 scope: customer + admin profile management (name, email-with-verify, password+HIBP, 2FA regen, backup codes regen, sessions list, "log out everywhere"), customer activity feed (filtered slice of audit_log), admin audit-log view + CSV export, all email triggers wired, i18n key audit, accessibility pass, full §11 acceptance checklist (excluding the two go-live items in M10) green.
