...
| **M11** Visual redesign + TOTP QR | 🟡 in scope, not started | — | Surfaced at the close of M10-C when the operator attempted bootstrap onboarding and refused: (a) public surface (welcome / login / login-2fa / reset / dashboard / admin / customer) is unstyled — Tailwind utilities against a thin tokens layer that doesn't carry through to the look-and-feel of the marketing site at `dbstudio.one`; (b) TOTP enrolment shows only the manual `otpauth://...` URI — no scannable QR. Both items must ship before v1.0.0 tag. Scope: full visual unification with `dbstudio.one` (fonts, type scale, palette, imagery, motion); server-side SVG QR rendering on the welcome enrolment + admin & customer TOTP regen views (no client JS, CSP-clean); a11y re-audit post-redesign. Spec at `docs/superpowers/specs/2026-04-30-m11-visual-redesign-design.md`. Estimated 1–2 focused days. |

**Latest commit on main:** `146f742` — `feat(m10-c): redirect / to /login post-coming-soon`.

**Resume here:** **M10 implementer-side and operator gates 10-A + 10-B complete; M10-C paused mid-flight before operator onboarding** because the unstyled public surface + the manual-only otpauth URI fall below the v1 acceptance bar. **M11 — visual redesign + server-side TOTP QR — must ship before v1.0.0 tag.** No production traffic on the portal yet (Cloudflare → NPM → portal works, /login is reachable, but no admin has consumed their welcome token). The operator's welcome URL and email are good for 7 days; we will re-issue if it expires before M11 lands. Next: open M11 with brainstorming → produce a plan via `superpowers:writing-plans` → execute. Survey-first: fetch `dbstudio.one`'s production CSS + asset list to understand the design system being matched, then map portal pages onto it. Server-side SVG QR is a separate small task (npm `qrcode`, render SVG string in EJS) and lands inside M11.

**Test/coverage state right now:** 565 tests green + 2 skipped (RUN_LIVE_EMAIL gate + RUN_PDF_E2E gate) across 67 files; with RUN_PDF_E2E=1 it's 9/9 NDA tests including the multi-page contract test. Smoke 5/5. Coverage gates pass for `lib/crypto/**` (100), `lib/auth/**` (80), `lib/nda.js` (80), `domain/credentials/**` (80), `domain/invoices/**` (75/80/60/75), `domain/ndas/**` (80/80/60/80), `lib/activity-feed.js` (95.83/85.71/100/100), `lib/audit-query.js` (89.74/79.54/100/94.11), `domain/customer-users/service.js` (86.51/77.65/96.66/89.16). Migrations 0001-0009 applied.
