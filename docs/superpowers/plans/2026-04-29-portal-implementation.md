...
| **M9** Profile + activity + polish | ✅ done | 2026-04-30 | 9.1 (6 customer profile sub-tasks: name, email-with-verify, password+HIBP, 2FA-TOTP regen, backup-codes regen, sessions list / revoke / log-out-everywhere — 35 service tests, all visible_to_customer audits). 9.2 (admin profile mirror — 11 service tests). 9.3 (broader customer activity feed in `lib/activity-feed.listActivityForCustomer` with metadata allow-list extended for M9 keys; routes/customer/activity + view; 7 tests). 9.4 (admin audit-log view + streamed CSV export in `lib/audit-query.{listAuditPage,streamAuditCsv}` — id-only cursor avoids the JS-Date µs-truncation pitfall, csv-escaping tested; 6 tests). 9.5 (`scripts/i18n-audit.js` — 620 offenders across 79 files; v1 ships EN-only, full localisation pass tracked in `docs/superpowers/follow-ups.md`). 9.6 (`scripts/a11y-check.js` static scanner reports 0 offenders for img-alt + heading-order + form-input-label; skip-link + focus-visible ring shipped; full axe-core run deferred per follow-ups). 9.7 (`docs/superpowers/m9-acceptance-dryrun.md` walks §11 line-by-line: 4/4 two-service split, 1/2 repo discipline (signed commits is operator policy), 1/1 NDA auditability, 5/6 M9 additions; the only acknowledged-not-green item is i18n which is deferred to v1.1). 9.X (M7 reviewer-deferred vault CRUD: customer-side credential list + create + delete via `routes/customer/credentials.js` — view-with-decrypt + edit deferred per follow-ups since they need the customer vault-unlock flow). Coming-soon polish: swap to logo-black so the brand shows on the white bg + skip-link infra. **Latent bug fixed in `lib/audit-query.js` design**: PG stores ts at µs precision, JS Date is ms; a strict-equality cursor on `(ts, id)` would silently skip rows when the JS-Date round-trip truncated µs to ms — solved by using id-only cursor (UUIDv7 ms-precision sorts identically). Twelve commits (`ff6327a..4ab2d98`) excluding the polish + push. 565 tests green + 2 skipped (RUN_LIVE_EMAIL + RUN_PDF_E2E gates) across 67 files. Smoke 5/5. Migration 0009_email_change_requests.sql applied. |
| **M10** Backups + go-live (gates) | — | — | |

**Latest commit on main:** `4ab2d98` — `feat(m9.6): a11y scaffolding — skip-link, focus ring, audit script` (further commits during M9 → M10 review-fix bundle will move this pointer).

**Resume here:** **M9 done end-to-end. Ready for M9 → M10 review checkpoint.** All sub-tasks of 9.1–9.7 + 9.X landed; 565 tests green + 2 skipped across 67 files; smoke.sh 5/5; the §11 acceptance dry-run lives at `docs/superpowers/m9-acceptance-dryrun.md` with two acknowledged-not-green items deferred to v1.1 / operator policy (i18n + signed commits). Next: run the M9 → M10 review checkpoint, address any reviewer feedback, then start M10 (backup + go-live operator gates).

**M8 review-deferred minors that fold into M9 polish naturally:** reviewer's M3 (defence-in-depth customerId on cancelByAdmin / markNeedsUpdate), M6 (route renders err.message verbatim), M7 (vault CRUD routes missing — closely related to M9 customer credential view path), M2/M5/M8/M10 (cosmetic). M7 review-deferred items also still pending under the same umbrella.

**Test/coverage state right now:** 500 tests green + 2 skipped (RUN_LIVE_EMAIL gate + RUN_PDF_E2E gate) across 58 files; with RUN_PDF_E2E=1 it's 9/9 NDA tests including the multi-page contract test. Smoke 5/5. Coverage gates pass for `lib/crypto/**` (100), `lib/auth/**` (80), `lib/nda.js` (80), `domain/credentials/**` (80), `domain/invoices/**` (75/80/60/75), `domain/ndas/**` (80/80/60/80). HTTP-layer route tests for invoice + NDA admin/customer routes are minimal (1-2 happy paths each) — service-layer coverage is the gate; M9 polish can backfill HTTP-layer rigor if needed.
