...
**Latest commit on main:** `54109e8` — `fix(logger): redact kek + bearer-token URL fields in object-form logs (M5)`.

**Resume here:** M5 → M6 review **cleared** (0C+3I+1 of 5M fixed in `0a3f189` `3bcf811` `234cefb` `54109e8`; 3 minors deferred — M1 journald URL-path tokens, M3 cosmetic expiresAt recompute, M4 err.code vs message regex). Begin M6 — Documents + projects (Tasks 6.1–6.5: `lib/files.js` storage path + signed download token, `domain/documents/{repo,service}.js` w/ multipart upload + magic-byte MIME sniff + filename sanitization + 50 MB/file + 5 GB/customer caps, admin upload UI w/ version chain via `parent_id`, customer download UI via 60s single-use signed URL, projects rolling documents into project containers). Run integration tests via `sudo bash scripts/run-tests.sh` (the wrapper stops portal.service so the live worker doesn't race the tests).

**Test/coverage state right now:** 280 tests green + 1 skipped (live-smoke RUN_LIVE_EMAIL gate) across 35 files, smoke.sh 5/5. Coverage on `domain/customers/service.js` 100/82.14/100/100 (branch gap = audit nullish-fallbacks, same defensive pattern the M4 review accepted on `worker.js`); `domain/customers/repo.js` partial — `findCustomerUserByEmail` and `updateCustomer` are scaffolding for customer login (post-checkpoint) and M9 profile editing and aren't exercised yet. All other coverage thresholds met as before. The ten M5 commits (range `10bf15c..54109e8`) are worth re-reading before M6 if you context-swap: `58f2a61` (timezone Europe/Madrid → Atlantic/Canary), `df5d4a9` (Task 5.1 transactional `service.create` w/ wrapped DEK), `a590428` (Task 5.2 admin customers list/new/detail routes), `515846b` (Task 5.3 customer onboarding + URL split + DEK-encrypted TOTP secret), `733a85e` (Task 5.4 dashboard stub + `requireCustomerSession` hoist), `5b361f3` (Task 5.5 suspend/reactivate/archive), `0a3f189` (review I1 race fix + status gate), `3bcf811` (review I3 onboarding error coverage), `234cefb` (review I2 route coverage + M2 `visible_to_customer`), `54109e8` (review M5 Pino redact paths).
